...
if result.loss_history:
    ...
    ax_conv.semilogy(iters, [1 - f for f in result.loss_history],
                     lw=1.8, color='steelblue')
    ax_conv.axhline(1 - result.fidelity, color='tomato', ls='--', lw=1.2,
...
for r, kl in enumerate(result.kappa_list):
    ax_kappa.bar(x_pos + r * width - 0.4 + width/2, np.abs(kl),
                 width=width, label=f"Ring {r+1}", alpha=0.85)
...
for r, kl in enumerate(result.kappa_list):
    theta = np.angle(kl)
    r_val = np.abs(kl)
    ax_phase_kappa.scatter(theta, r_val, s=60, color=colours[r],
                           label=f"Ring {r+1}", zorder=3)
...
ax_phase_kappa.legend(fontsize=7, loc='upper right',
...
def _matrix_panel(ax, data, title, cmap, vmin, vmax, clabel):
    ...
    ax.set_xticks(range(N)); ax.set_xticklabels(lbls, fontsize=7)
...
_matrix_panel(ax_diff, np.abs(U_target - M_opt), "|U - M|  (error)", "hot",    0, 0.5, "|·|")
...
plt.savefig("haar_inverse_design.png", dpi=150, bbox_inches="tight")
print("\nFigure saved to haar_inverse_design.png")
plt.show()
